...
class Inception_Conv(nn.Module):
    # Inception Conv
    def __init__(self, c1, c2, k=3, s=2, g=1, p=None, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
    ...
    def forward(self, x):
        x1 = self.act(self.bn(self.conv1(x)))
        x2 = self.act(self.bn(self.conv2(x)))
        x = torch.add(x1, x2)
        return x
